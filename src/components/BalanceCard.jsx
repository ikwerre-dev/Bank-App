...
const BalanceCard = ({ type, amount = 0 }) => {
      ...
      const fetchData = async () => {
        try {
          ...
          if (response.data && response.data.data) {
            const { is_withdrawn, status, task_day, amount } =
              response.data.data;
            ...
            if (status !== 0) {
              ...
              const calculatedAmount =
                parseInt(task_day) * 0.2 * parseInt(amount) + parseInt(amount);
  ...
  return (
    <div
      ...
      style={{
        background: "linear-gradient(135deg, #2d5674, #308966)", // Adjust colors for a smooth gradient
    ...
    >
      ...
      <h2 className="text-3xl font-bold text-start px-3 py-2 mb-1">
        $
