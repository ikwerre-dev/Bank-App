...
function App() {
  ...
  const openIntiatePaymentModal = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}deposit`,
        {
  ...
  const PaymentMethod = ({
    ...
    return (
      <div className="flex border border-sm rounded-2xl border-blue-200 items-center py-5 px-6 cursor-pointer">
        <div className="flex-grow">
          <p
            className={`text-sm ${isSelected ? "text-blue-600" : "text-gray-700"}`}
          >
            {name}
          </p>
          ...
          <p
            onClick={handleCopy}
            className="text-sm flex justify-between text-black mt-5"
          >
            {data}
          ...
          <p className="text-xs my-3 flex gap-2 text-gray-500">
        ...
        <>
          ...
          <div className="bg-white text-black p-4 px-6 rounded-t-3xl mt-4 flex-grow">
            ...
            <div className=" h-100 grid relative overflow-hidden">
              <p>
                Contact Support to Process your deposit or Share your Account
                Number to Another User on this bank to do an internal Transfer
              </p>
            </div>
          </div>
        </>
